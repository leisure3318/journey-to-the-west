...
- [x] 5 种地形区域：草原（中心）、山林（西北）、水泽（东北）、沙漠（西南）、火域（东南）
- [x] TerrainSystem：区域叠加渲染（色彩叠层 + 地面细节：暗斑/沙纹/水洼/岩浆裂缝）
- [x] 9 种新装饰纹理：松树、仙人掌、枯树、芦苇、岩浆石、蘑菇、水池、余烬、沙丘
- [x] DecorationSystem 重写：350 个装饰物按区域分配，区域专属配色
- [x] 新增 3 个 Boss：黑熊精(HP2500)、蜘蛛精(HP4500)、金角大王(HP6000)
- [x] Boss 总数 3→6，分布在不同区域（黑风山/黄风岭/白虎岭/盘丝洞/平顶山/火云洞）
- [x] POI 定向放置（收徒点靠近中心草原，Boss 分布在各区域）
- [x] 小地图显示区域颜色（草绿/深绿/蓝绿/土黄/暗红）
- [x] 迷雾适配大地图（单元格 50→80px，揭雾半径 250→320）
- [x] 宝箱数量适配（8-12 → 15-25 个/局）
- [x] 刷怪上限适配（各阶段上调 30-35%）
- [x] Boss 宝物系统（BossLootSystem）：击杀 Boss 掉落随机宝物
  - 每个 Boss 对应原著主题宝物（定风珠/黑风令/白骨如意/蛛丝灵链/紫金红葫芦/三昧火珠）
  - 2-3 条随机永久属性（移速/暴击/HP/减伤/护盾等 10 种属性池）
  - 属性值随机范围（如移速+8%-20%，暴击+5%-15%）
- [x] 宝物展示弹窗（击杀后居中显示图标+名称+属性列表，3.5秒淡出）
- [x] 右侧宝物栏 RelicBar（图标槽+悬浮 tooltip 显示属性详情）
- [x] 敌人渐进增强：HP/伤害每分钟+4%，移速随时间微增，刷怪上限缓慢提升
- [x] TODO.md 更新至 Day6

**代码变更统计：**
- 新增文件3个：TerrainSystem.ts, BossLootSystem.ts, RelicBar.ts
- 修改文件10个：GameConfig.ts, MapConfig.ts, EnemyConfig.ts, BootScene.ts, GameScene.ts, DecorationSystem.ts, FogOfWar.ts, EnemySpawner.ts, ChestSystem.ts, MiniMap.ts

**未开始的事项：**
- [ ] POI 场景图片（image-2 生成 10 张，忠实原著）
- [ ] 更多敌人行为（召唤型、环境型、精英标识）
- [ ] 章节关卡制
...
**完成事项：**
- [x] POI 场景图片圆形融合（Canvas radial fade，边缘渐隐融入地形）
- [x] 宝物栏位置修复（避免与小地图重叠，buff 属性常驻显示）
- [x] 设置面板：暂停菜单内嵌音量滑块（拖拽调节）+ 静音按钮
  - SoundManager 新增 setVolume/volume API
  - PauseMenu 升级：音量滑块 + 静音切换 + 视觉反馈
- [x] 环境障碍物系统（ObstacleSystem）
  - 70 个可碰撞障碍物，按区域分布
  - 玩家和敌人均碰撞（物理 StaticGroup）
  - 碰撞体积缩小至视觉 50%，手感宽容
- [x] 伤害飘字对象池（CombatSystem）
  - 40 个 Text 对象预创建循环复用
  - 不再每帧创建/销毁，减少 GC 压力

**代码变更统计：**
- 新增文件1个：ObstacleSystem.ts
- 修改文件5个：SoundManager.ts, PauseMenu.ts, GameScene.ts, CombatSystem.ts, LandmarkSystem.ts
