...
export class VictoryPanel {
  ...
  show(stats: VictoryStats) {
    ...
    this.scene.add.rectangle(400, 300, 440, 340, 0x000000, 0.85)
      .setScrollFactor(0).setDepth(1000);

    this.scene.add.text(400, 195, "取经成功！", {
      fontSize: "32px", color: "#ffdd44", fontStyle: "bold",
    }).setOrigin(0.5).setScrollFactor(0).setDepth(1001);

    this.scene.add.text(400, 240, `击败 ${stats.bossName}`, {
      fontSize: "18px", color: "#ff8844",
    }).setOrigin(0.5).setScrollFactor(0).setDepth(1001);

    this.scene.add.text(400, 275,
      `存活 ${m}:${s.toString().padStart(2, "0")}  |  击杀 ${stats.kills}  |  Lv ${stats.level}`, {
      fontSize: "16px", color: "#ffffff",
    }).setOrigin(0.5).setScrollFactor(0).setDepth(1001);

    const defeated = saveSystem.bossesDefeated;
    const records = [
      `已征服 ${defeated.length}/6 Boss`,
      `最佳存活 ${saveSystem.formatTime(saveSystem.bestTimeMs)}`,
      `最高击杀 ${saveSystem.bestKills}  |  总场次 ${saveSystem.totalRuns}`,
    ].join("\n");
    this.scene.add.text(400, 330, records, {
      fontSize: "12px", color: "#888888", align: "center", lineSpacing: 6,
    }).setOrigin(0.5).setScrollFactor(0).setDepth(1001);

    const restart = this.scene.add.text(340, 400, "[ 再来一次 ]", {
    ...
    const menu = this.scene.add.text(460, 400, "[ 主菜单 ]", {
